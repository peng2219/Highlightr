...
open class Highlightr {
    ...
    private let htmlEscape = try! NSRegularExpression(
        pattern: "&#?[a-zA-Z0-9]+?;", options: .caseInsensitive)
    ...
    public init?(highlightPath: String? = nil) {
        ...
        #endif
        self.bundle = bundle
        guard let hgPath = highlightPath ?? bundle.path(forResource: "highlight.min", ofType: "js")
        else {
            return nil
        }
        ...
        -> NSAttributedString?
    {
        ...
        if let languageName = languageName {
            let result: JSValue = hljs.invokeMethod(
                "highlight", withArguments: [languageName, code, ignoreIllegals])
        ...
        if fastRender {
            ...
            string =
                "<style>" + theme.lightTheme + "</style><pre><code class=\"hljs\">" + string
                + "</code></pre>"
            ...
            safeMainSync {
                returnString = try? NSMutableAttributedString(
                    data: data, options: opt, documentAttributes: nil)
            }
        }
    ...
    open func detectLanguage(_ code: String) -> String? {
        // 调用highlight.js的语言检测方法
        let result = hljs.invokeMethod("highlightAuto", withArguments: [code])

        // 从结果中获取检测到的语言
        guard let language = result?.objectForKeyedSubscript("language").toString() else {
            return nil
        }
    ...
    private func processHTMLString(_ string: String) -> NSAttributedString? {
        ...
        while !scanner.isAtEnd {
            ...
            if scannedString != nil && scannedString!.length > 0 {
                let attrScannedString = theme.applyStyleToString(
                    scannedString! as String, styleList: propStack)
